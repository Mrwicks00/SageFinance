...
plugins.command("add").alias("install").description("Add a plugin to the project").argument("<plugin>", 'plugins name (e.g., "abc", "plugin-abc", "elizaos/plugin-abc")').option("-s, --skip-env-prompt", "Skip prompting for environment variables").option("--skip-verification", "Skip plugin import verification after installation").option("-b, --branch <branchName>", "Branch to install from when using monorepo source", "main").option("-T, --tag <tagname>", "Specify a tag to install (e.g., beta)").action(async (pluginArg, opts) => {
  try {
...
async function installPluginDependencies(projectInfo) {
  ...
  if (project.isPlugin && project.pluginModule?.dependencies && project.pluginModule.dependencies.length > 0) {
    ...
    const { installPlugin: installPlugin2 } = await import("./utils-WRGBZBAJ.js");
